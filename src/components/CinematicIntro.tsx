...
export default function CinematicIntro() {
  ...
  useEffect(() => {
    // ปรับเวลาแสดงผลให้กระชับขึ้น (ประมาณ 2.2 วินาที)
    ...
    <AnimatePresence>
      {showIntro && (
        ...
        >
          ...
          {/* กล่องข้อความ Blockbuster Title */}
          <div className="relative text-center px-4 max-w-7xl mx-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 0.7, scale: 1.2 }}
              transition={{ duration: 2, ease: "easeOut" }}
              className="absolute inset-0 -z-10 bg-gradient-to-r from-cyan-500/40 via-sky-300/30 to-blue-600/40 blur-[100px] rounded-full pointer-events-none"
            />
            ...
            <motion.h1
              ...
              className="font-[950] text-5xl sm:text-7xl md:text-9xl lg:text-[10rem] uppercase leading-[0.88] tracking-tight text-white [-webkit-text-stroke:1px_rgba(255,255,255,0.3)] [filter:drop-shadow(0_0_35px_rgba(6,182,212,0.8))]"
            >
              DIGITAL
              <br />
          ...
          <motion.button
            ...
            className="absolute bottom-8 right-8 font-mono text-xs tracking-widest text-slate-400 hover:text-cyan-400 uppercase transition-all px-3 py-1.5 rounded-lg border border-white/10 bg-white/5"
          >
            [ SKIP INTRO → ]
          </motion.button>
        </motion.div>
